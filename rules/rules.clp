...
(defrule start_road_maintenance
    ...
    =>
    (bind ?road_id (str-replace (nth$ 3 (fact-slot-value ?t vals)) "r_" ""))
    (do-for-fact ((?r road)) (eq ?r:road_id ?road_id) (println "Starting road maintenance on road " ?r:name))
)
...
(defrule start_building_maintenance
    ...
    =>
    (bind ?building_id (str-replace (nth$ 1 (fact-slot-value ?t vals)) "b_" ""))
    (do-for-fact ((?b building)) (eq ?b:building_id ?building_id) (println "Starting building maintenance on building " ?b:name))
)
...
(deffunction sensor_data (?sensor ?sensor_type ?time ?data)
    ...
    (if (eq (fact-slot-value ?sensor_type name) "matera_station_type0")
        ...
        (if (>= ?pm10 40)
            then
            (println "Received data from sensor " (fact-slot-value ?sensor name) " with PM10 value " ?pm10)
            (do-for-all-facts ((?u user)) (eq ?u:user_role technician)
                (send_message warning ?u:user_id (str-cat "Il sensore " (fact-slot-value ?sensor name) " ha rilevato un valore di PM10 pari a " ?pm10 ".") (create$ "Ok") (fact-slot-value ?sensor location))
            )
        )
        (if (>= ?pm25 25)
            then
            (println "Received data from sensor " (fact-slot-value ?sensor name) " with PM2.5 value " ?pm25)
            (do-for-all-facts ((?u user)) (eq ?u:user_role technician)
                (send_message warning ?u:user_id (str-cat "Il sensore " (fact-slot-value ?sensor name) " ha rilevato un valore di PM2.5 pari a " ?pm25 ".") (create$ "Ok") (fact-slot-value ?sensor location))
            )
        )
    )
)
...
(deffunction answer_message (?m_id ?answer)
    (do-for-fact ((?m message)) (eq ?m:message_id ?m_id)
        (modify ?m (answer ?answer))
    )
)